...
# ultimate_benchmark_gemini_flash.sh
# The definitive stress test for intent tracking and noise filtering.
# Runs the ULTIMATE BENCHMARK against Gemini 2.5 Flash via local proxy.

API_URL="http://192.168.0.113:8000/v1/chat/completions"
API_KEY="VerysecretKey"
MODEL="gemini_cli/gemini-2.5-flash"
SUMMARY_FILE="ultimate_benchmark_gemini_flash_results.md"

# Clear previous results
echo "# Voxtype Ultimate Benchmark: GEMINI 2.5 FLASH (RFC Optimized)" > "$SUMMARY_FILE"
echo "Date: $(date)" >> "$SUMMARY_FILE"
echo "Model: $MODEL" >> "$SUMMARY_FILE"
echo "Goal: Stress test intent tracking with STRICT RFC 2119 prompts." >> "$SUMMARY_FILE"
echo "" >> "$SUMMARY_FILE"
